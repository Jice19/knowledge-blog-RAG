...
export default function ArticlePage() {
  ...
  if (loading) {
    return <div className="mx-auto max-w-3xl px-4 py-24 text-center text-slate-400">加载中…</div>
  }
  if (!article) {
      ...
      <header className="mt-6">
        ...
        <div className="mt-4 flex flex-wrap items-center gap-3 text-sm text-slate-400">
          <span className="flex items-center gap-1.5">
            ...
            <span className="text-slate-600">{article.authorName}</span>
          </span>
          <span>·</span>
      ...
      {article.cover ? (
        ...
        <div
          className={`mt-8 flex h-48 w-full items-center justify-center rounded-2xl bg-gradient-to-br ${coverGradient(
            article.categorySlug ?? undefined,
          )}`}
        >
          <span className="text-6xl font-bold text-white/90">{article.categoryName?.[0] ?? '文'}</span>
        </div>
      )}
